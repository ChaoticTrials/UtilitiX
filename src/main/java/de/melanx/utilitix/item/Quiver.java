...
public class Quiver extends ItemBase implements Registerable {
    ...
    public static final ResourceLocation SLOT_VALIDATOR = ResourceLocation.fromNamespaceAndPath(UtilitiX.getInstance().modid, "quiver_arrows");
...
//        GenericMenu.registerSlotValidator(SLOT_VALIDATOR, (slot, stack) -> stack.is(ItemTags.ARROWS)); todo
    }
...
//                        stack.getOrCreateTag().put("Items", handler.get().serializeNBT()); todo
                        player.setItemInHand(hand, stack);
                    })
                    .validator(stack1 -> stack1.is(ItemTags.ARROWS), 0, 1, 2, 3, 4, 5, 6, 7, 8)
                    .build());
            if (stack.has(DataComponents.CONTAINER)) {
//                handler.get().deserializeNBT(stack.getOrCreateTag().getCompound("Items")); todo
            }

            if (player instanceof ServerPlayer) {
//                GenericMenu.open((ServerPlayer) player, handler.get(), Component.translatable("screen." + UtilitiX.getInstance().modid + ".quiver"), SLOT_VALIDATOR); todo
            }
...
//    }
    ...
    public static IItemHandlerModifiable getInventory(ItemStack stack) {
        ...
        AtomicReference<BaseItemStackHandler> handler = new AtomicReference<>(null);
        handler.set(BaseItemStackHandler.builder(9)
//                .contentsChanged(slot -> stack.getOrCreateTag().put("Items", handler.get().serializeNBT())) todo
                .validator(stack1 -> stack1.is(ItemTags.ARROWS))
                .build()
        );
        // todo use ContainerHelper.load or .save
